...
class Hooks:
    ...
    _BRANCH_TYPES = ['b', 'bl', 'ba', 'bla']

    def make_branch_insn(from_addr: int, to_addr: int, branch_type: str) -> int:
        if branch_type not in Hooks._BRANCH_TYPES:
            raise ValueError('invalid branch type: %s' % branch_type)

        extra = Hooks._BRANCH_TYPES.index(branch_type)
        ...
        if distance >= 0x2000000 or distance <= -0x2000000:
            raise ValueError('branching too far: %08x to %08x' % (from_addr, to_addr))
    ...
    class Hook(object):
        ...
        def __init__(self, builder, module, data) -> None:
            """Sets up a hook"""
            ...
            if self.has_context:
                hook_type = type(self)
                self.context = builder._hook_contexts[hook_type]
    ...
    class BranchInsnHook(Hook):
        ...
        def create_patches(self) -> None:
            ...
            if is_symbol_name and self.builder.dynamic_link:
                branch_insn = Hooks.make_branch_insn(src_addr, -1, self.data['branch_type'])
                self.builder.add_patch(src_addr, Hooks._u32.pack(branch_insn))

                dylink = self.builder.dynamic_link
                dylink.add_reloc(dylink.R_PPC_REL24, src_addr, target_func)
            else:
                branch_insn = Hooks.make_branch_insn(src_addr, target_func, self.data['branch_type'])
                self.builder.add_patch(src_addr, Hooks._u32.pack(branch_insn))
    ...
    class AddFunctionPointerHook(Hook):
        ...
        def create_patches(self) -> None:
            ...
            if is_symbol_name and self.builder.dynamic_link:
                ...
                self.builder.add_patch(src_addr, self._u32.pack(target_func))
    ...
    class NopInsnHook(Hook):
        ...
        def create_patches(self) -> None:
            ...
            if isinstance(area, list):
                addr, end = area
                count = (end + 4 - addr) // 4
                nop_patch = b'\x60\x00\x00\x00' * count
            else:
                addr = area
                nop_patch = b'\x60\x00\x00\x00'
