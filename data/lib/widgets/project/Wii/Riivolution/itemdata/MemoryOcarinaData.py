...
class MemoryOcarinaData(BaseSubItemData):
    ...
    def init(app: QBaseApplication) -> None:
        MemoryOcarinaData._lang = app.get_lang_data('QMainWindow.QSlidingStackedWidget.mainMenu.projects.projectWidget.Wii.RiivolutionWidget.WiiDiscWidget.PatchData.PropertyWidget.MemoryData.MemoryOcarinaData')
    ...
    def __init__(self, data: MemoryOcarina, path: str) -> None:
        ...
        subframe.layout_.addWidget(self._comment_lineedit, 2, 0)
    ...
    def _update_text(self) -> None:
        s = self._lang.get('QLabel.text').replace('%s', f'0x{self._data.offset:X}', 1).replace('%s', f'0x{self._data.value:X}', 1)
